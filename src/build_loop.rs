...
impl<'a> BuildLoop<'a> {
    ...
    pub fn forever(&mut self, tx: chan::Sender<LoopHandlerEvent>, rx_ping: chan::Receiver<()>) {
        ...
        let maybe_get_one_reason_to_build = |self_: &Self, output_paths: &Option<builder::OutputPaths<_>>, blocking: bool| -> Option<StartOrder> {
            let timeout = if blocking { chan::never() } else { chan::after(Duration::from_millis(1)) };
            chan::select! {
        ...
        loop {
            if to_start.is_none() {
                to_start = maybe_get_one_reason_to_build(self, &output_paths, true /* blocking */);
            }
            ...
            if to_start.is_some() {
                ...
                loop {
                    to_start = maybe_get_one_reason_to_build(self, &output_paths, false /* blocking */);
                    match &to_start {
                        Some(StartOrder{nix_file, ..}) if nix_file == &first_trigger.nix_file => {
